...
const Fruit = () => {
  ...
  const action = `${host}/roblox/fruit`
  ...
  function handdleButton(e) {
    ...
    if (!token) {
      ...
      fetch(`${host}/updatebillcaythue/${totalPrice}/Fruit`, {
        method: "POST",
        headers: {
          Authorization: `${token}`,
          ...
          <Row gutter={[16, 16]}>
            {products.map((product) => (
              <Col key={product._id} xs={24} sm={12} md={8} lg={6} xl={6} >
                {product.imgFruit.map((item, index) => (
                  <Card
                    hoverable
                    ...
                    cover={
                      <img
                        src={item}
                        alt={product.name}
                        style={{ height: "180px", objectFit: "cover", padding: "17px", borderRadius: "25px" }}
                      />
                    }
                  >
                    <Card.Meta
                    ...
                    />
                    <Button color="cyan" variant="solid" htmlType="submit" onClick={handdleClick} title={product.price} style={{ marginTop: 20 }}>
                      Mua Ngay
                    </Button>
                  </Card>
                ))}
