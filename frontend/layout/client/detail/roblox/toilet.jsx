...
const Toilet = () => {
  ...
  const action = `${host}/roblox/toilet`
  ...
  function handdleButton(e) {
    ...
    if (!token) {
      ...
      fetch(`${host}/updatebillcaythue/${totalPrice}/Toilet`, {
        method: "POST",
        headers: {
          Authorization: `${token}`,
  ...
  return (
    <>
      <form action="" onSubmit={handdleButton}>
        <div style={{ padding: "20px 175px"}}>
          <Row gutter={[16, 16]}>
            {products.map((product) => (
              <Col key={product._id} xs={24} sm={12} md={8} lg={6} xl={6} >
                {product.imgToilet.map((item, index) => (
                  <Card
                    hoverable
                    ...
                    cover={
                      <img
                        src={item}
                        alt={product.name}
                        style={{ height: "200px", objectFit: "cover", padding: "17px", borderRadius: "25px" }}
                      />
                    }
                  >
                    <Card.Meta
                    ...
                    />
                    <Button color="default" variant="solid" htmlType="submit" onClick={handdleClick} title={product.price} style={{ marginTop: 20 }}>
                      Mua Ngay
                    </Button>
                  </Card>
                ))}
